...
function calculateStats(dataArr) {
  ...
  const pendingReview = dataArr.filter(
    (property) => property.status.toLowerCase() === "pending"
  ).length;
  const completedReview = dataArr.filter(
    ...
    .map(
      (stat) => `
        <div class="stat-card">
            ...
            ${
              stat.trend
                ? `<div class="trend ${stat.isPositive ? "positive" : ""}">${
                    stat.trend
                  }</div>`
                : ""
            }
      ...
      return `
        <tr>
            ...
            <td class="action-buttons">
                <button class="btn btn-edit" onclick="editProperty(this, '${
                  property.id
                }')" data-edit-url="${property.reference}"  >
                     ${
                       property.status.toLowerCase() == "completed"
                         ? "Update"
                         : "Edit"
...
document
  .getElementById("viewConstruction")
