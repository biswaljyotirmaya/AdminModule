...
function initMap() {
  dataref
    .once("value", function (snapshot) {
      ...
      var measurementSettings = data.measurementSettings || {
        oneInchIntoFeet: 10,
      };
      ...
      document.getElementById(
        "scaleDisplay"
      ).textContent = `1 inch = ${scale.inchToFeet} feet`;
      ...
      image.onload = () => {
        ...
        if (workedGeojson) {
          try {
            const geojsonData =
              typeof workedGeojson === "string"
                ? JSON.parse(workedGeojson)
                : workedGeojson;
            ...
            features.forEach((feature) => {
              ...
              if (!vectorLayers[layerType]) {
                console.warn(
                  `Layer ${layerType} not found in active layers, skipping feature.`
                );
                return;
              }
              ...
              const featureId =
                feature.getId() || `feature_${featureCounter++}`;
            ...
            const allFeaturesExtent = new ol.source.Vector({
              features,
            }).getExtent();
            if (!ol.extent.isEmpty(allFeaturesExtent)) {
              map.getView().fit(allFeaturesExtent, {
                padding: [50, 50, 50, 50],
                maxZoom: 8,
              });
            }
...
function updateMeasurementDisplay(layerType) {
  ...
  if (measurementEl) {
    ...
    console.log(
      `Updating display for ${layerType}: ${value.toFixed(2)} ${
    ...
    measurementEl.textContent = `${value.toFixed(2)} ${
      spec.geometryType === "Polygon"
        ? "sq ft"
        : spec.geometryType === "LineString"
        ? "ft"
...
function calculateMeasurement(feature, spec) {
  ...
  if (spec.geometryType === "Polygon") {
    measurement =
      geometry.getArea() * scale.inchToFeet ** 2 * calibrationFactor;
  } else if (spec.geometryType === "LineString") {
    measurement = geometry.getLength() * scale.inchToFeet;
  } else if (spec.geometryType === "Point") {
    measurement = 1;
...
function startDeleting(layerType) {
  ...
  select.on("select", (event) => {
    ...
    if (selectedFeatures.length > 0) {
      ...
      console.log(
        `Deleting feature ${featureId} from ${layerType}, measurement: ${measurement}`
      );
      ...
      console.log(
        `Updated measurements[${layerType}]: ${measurements[layerType]}`
      );
      updateMeasurementDisplay(layerType);
...
function startEditing(layerType) {
  ...
  modify.on("modifystart", (event) => {
    ...
    if (features.length > 0) {
      ...
      console.log(
        `Started editing feature ${features[0].getId()} in ${layerType}`
      );
    }
  });
  ...
  modify.on("modifyend", (event) => {
    ...
    if (features.length > 0 && oldFeature) {
      ...
      console.log(
        `Editing feature ${featureId} in ${layerType}: old measurement = ${oldMeasurement}, new measurement = ${newMeasurement}`
      );
...
function undo() {
  ...
  if (action === "add" && source.getFeatureById(featureId)) {
    ...
    console.log(
      `Undoing add for feature ${featureId} in ${layerType}, measurement: ${measurementValue}`
    );
    ...
    console.log(
      `Undo - Updated measurements[${layerType}]: ${measurements[layerType]}`
    );
    updateMeasurementDisplay(layerType);
    ...
    const measurementValue =
      measurement || calculateMeasurement(newFeature, spec);
    console.log(
      `Undoing delete for feature ${featureId} in ${layerType}, measurement: ${measurementValue}`
    );
    ...
    if (feature) {
      console.log(
        `Undoing edit for feature ${featureId} in ${layerType}: oldMeasurement = ${oldMeasurement}, newMeasurement = ${newMeasurement}`
      );
      ...
      measurements[layerType] =
        (measurements[layerType] || 0) - newMeasurement + oldMeasurement;
      if (measurements[layerType] <= 0) measurements[layerType] = 0;
      console.log(
        `Undo - Updated measurements[${layerType}]: ${measurements[layerType]}`
      );
...
function redo() {
  ...
  if (action === "add") {
    ...
    const measurementValue =
      measurement || calculateMeasurement(newFeature, spec);
    console.log(
      `Redoing add for feature ${featureId} in ${layerType}, measurement: ${measurementValue}`
    );

    measurements[layerType] = (measurements[layerType] || 0) + measurementValue;
    console.log(
      `Redo - Updated measurements[${layerType}]: ${measurements[layerType]}`
    );
    updateMeasurementDisplay(layerType);
    ...
    if (featureToDelete) {
      const measurementValue =
        measurement || calculateMeasurement(featureToDelete, spec);
      console.log(
        `Redoing delete for feature ${featureId} in ${layerType}, measurement: ${measurementValue}`
      );

      source.removeFeature(featureToDelete);
      measurements[layerType] =
        (measurements[layerType] || 0) - measurementValue;
      if (measurements[layerType] <= 0) measurements[layerType] = 0;
      console.log(
        `Redo - Updated measurements[${layerType}]: ${measurements[layerType]}`
      );
    ...
    if (feature) {
      console.log(
        `Redoing edit for feature ${featureId} in ${layerType}: oldMeasurement = ${oldMeasurement}, newMeasurement = ${newMeasurement}`
      );
