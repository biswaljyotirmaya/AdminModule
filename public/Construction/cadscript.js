  ...
  dataref
    .once("value", function (snapshot) {
      ...
      var measurementSettings = data.measurementSettings || {
        oneInchIntoFeet: 10,
      }; // Measurement settings from Firebase
...
function startDrawing(layerType) {
  ...
  document
    .querySelector(`button[data-layer="${layerType}"]`)
  ...
  draw.on("drawend", (event) => {
    ...
    if (spec.geometryType === "Polygon") {
      measurement =
        ol.sphere.getArea(geometry) * (scale.inchToFeet / scale.ratio) ** 2;
    } else if (spec.geometryType === "LineString") {
      measurement =
        ol.sphere.getLength(geometry) * (scale.inchToFeet / scale.ratio);
    }
...
function updateMeasurementDisplay(layerType) {
  ...
  if (measurementEl && measurements[layerType]) {
    measurementEl.textContent = `${measurements[layerType].toFixed(2)} ${
      spec.geometryType === "Polygon" ? "sq ft" : "ft"
    }`;
  }
}
...
function cancelDrawing() {
  ...
  if (selectedLayer) {
    document
      .querySelector(`button[data-layer="${selectedLayer}"]`)
...
function handleSave() {
  ...
  function submitDataFinal() {
    if (confirm("Are you sure you have completed the data correctly?")) {
      ...
      dataref
        .update({ status: "Completed", workedGeojson: GetGeoJson })
        .then(() => {
          document.cookie = "";
