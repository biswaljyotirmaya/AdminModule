...
function isValidPolygon(coordinates) {
  ...
  return (
    ring &&
    Array.isArray(ring) &&
    ring.length >= 4 && // At least 4 points
...
function initMap() {
  ...
  dataref
    ...
    .then(async function (snapshot) {
      ...
      if (!data) {
        throw new Error(
          "No data found in Firebase for reference: " + reference
        );
      }
      ...
      const boundaryLayers = data.boundaryLayers || [data.boundary];
      ...
      dynamicLayerTypes.forEach((layerType) => {
        const feature = workedGeojson.features.find(
          (f) => f.properties.layerType === layerType
        );
        if (!LAYERS_SPECS[layerType]) {
          ...
          LAYERS_SPECS[layerType].color =
            feature?.properties?.style?.color || getRandomColor();
        }
      });

      const activeLayers =
        layersFromDb.length > 0
          ? Object.fromEntries(
              Object.entries(LAYERS_SPECS).filter(([layerType]) =>
                layersFromDb.includes(layerType)
              )
            )
          : Object.fromEntries(Object.entries(LAYERS_SPECS));
      boundaryLayers.forEach((boundary, index) => {
        const layerType = `boundary_${index + 1}`;
        LAYERS_SPECS[layerType] = {
          label: `Boundary ${index + 1}`,
          icon: "■",
          color: boundary.properties?.stroke || "#00ff33",
          geometryType: "Polygon",
        };
      });

      // Step 5: Initialize tile layer
      ...
      // Step 6: Initialize vector layers for active layers
      ...
      // Step 7: Initialize vector layers for boundaries
      let extent = null;
      boundaryLayers.forEach((boundary, index) => {
        try {
          if (
            boundary.geometry.type === "Polygon" &&
            !isValidPolygon(boundary.geometry.coordinates)
          ) {
            console.warn(
              `Invalid boundary polygon at index ${index + 1}, skipping:`,
              boundary
            );
            return;
          }
          const format = new ol.format.GeoJSON();
          const feature = format.readFeature(boundary, {
            dataProjection: "EPSG:4326",
            featureProjection: "EPSG:3857",
          });
          feature.set("layerType", `boundary_${index + 1}`);
          const layerType = `boundary_${index + 1}`;
          ...
          const vectorLayer = new ol.layer.Vector({
            ...
            style: new ol.style.Style({
              stroke: new ol.style.Stroke({
                color: boundary.properties?.stroke || "#00ff33",
                width: boundary.properties?.["stroke-width"] || 3,
              }),
              fill: new ol.style.Fill({
                color:
                  boundary.properties?.fill +
                  (boundary.properties?.["fill-opacity"]
                    ? Math.round(boundary.properties["fill-opacity"] * 255)
                        .toString(16)
                        .padStart(2, "0")
                    : "40"),
              }),
          ...
          vectorLayers[layerType] = vectorLayer;
          map ? map.addLayer(vectorLayer) : null;
          ...
          console.error(
            `Error processing boundary at index ${index + 1}:`,
            error,
            boundary
          );
        }
      ...
      // Step 9: Fit map to boundaries if they exist
      if (extent) {
        map.getView().fit(extent, { padding: [50, 50, 50, 50], maxZoom: 14 });
      }

      // Step 10: Load workedGeojson
      if (workedGeojson) {
        try {
          const format = new ol.format.GeoJSON();
          const features = format.readFeatures(workedGeojson, {
            dataProjection: "EPSG:4326",
            featureProjection: "EPSG:3857",
          });
          ...
          features.forEach((feature) => {
            ...
            if (
              ...
              console.warn(
                `Invalid polygon feature in workedGeojson, skipping:`,
            ...
            if (spec && spec.geometryType === "Polygon") {
              ...
              const coords = geometry
                ...
                .map((c) => ol.proj.toLonLat(c));
              measurement =
                turf.length(turf.lineString(coords), { units: "meters" }) *
                3.28084; // ft
            } else if (spec && spec.geometryType === "Point") {
              measurement = 1;
            }
            measurements[layerType] =
              (measurements[layerType] || 0) + measurement;
            updateMeasurementDisplay(layerType);
            history.push({ layerType, featureId, action: "add" });
          });
        } catch (error) {
          console.error(
            "Error processing workedGeojson:",
            error,
            workedGeojson
          );
        }
      ...
      // Step 11: Initialize layers list with boundaries first, then active layers
      const boundaryLayersMap = Object.fromEntries(
        Object.entries(LAYERS_SPECS).filter(([key]) =>
          key.startsWith("boundary_")
        )
      );
      const allLayers = { ...boundaryLayersMap, ...activeLayers };
      initLayersList(allLayers);
      initControls();
    ...
    .catch((error) => {
      ...
      const allLayers = { ...boundaryLayersMap, ...LAYERS_SPECS };
      initLayersList(allLayers);
      initControls();
      initKeyboardListeners();
    })
...
function changeLayerColor() {
  ...
  const boundaryLayersMap = Object.fromEntries(
    Object.entries(LAYERS_SPECS).filter(([key]) => key.startsWith("boundary_"))
  );
  const allLayers = {
    ...boundaryLayersMap,
    ...Object.fromEntries(
      Object.entries(LAYERS_SPECS).filter(
        ([layerType]) =>
          vectorLayers[layerType] && !layerType.startsWith("boundary_")
      )
    ),
  };
  initLayersList(allLayers);
...
function zoomToBoundary(layerType) {
  const vectorLayer = vectorLayers[layerType];
  if (!vectorLayer) {
    console.warn(`No vector layer found for ${layerType}`);
    return;
  }
  const source = vectorLayer.getSource();
  const features = source.getFeatures();
  if (features.length === 0) {
    console.warn(`No features found for ${layerType}`);
    return;
  }
  const extent = source.getExtent();
  if (ol.extent.isEmpty(extent)) {
    console.warn(`Empty extent for ${layerType}`);
    return;
  }
  map
    .getView()
    .fit(extent, { padding: [50, 50, 50, 50], maxZoom: 18, duration: 500 });
}
function initLayersList(activeLayers) {
  const layersList = document.getElementById("layersList");
  layersList.innerHTML = "";

  // Sort layers to put boundaries first
  const sortedLayers = Object.entries(activeLayers).sort(([a], [b]) => {
    const isBoundaryA = a.startsWith("boundary_");
    const isBoundaryB = b.startsWith("boundary_");
    if (isBoundaryA && !isBoundaryB) return -1;
    if (!isBoundaryA && isBoundaryB) return 1;
    return a.localeCompare(b);
  });

  // --- Create boundary dropdown once ---
  const boundaryDropdownWrapper = document.createElement("div");
  boundaryDropdownWrapper.className = "boundary-dropdown";
  boundaryDropdownWrapper.innerHTML = `
       <div class="layer-item boundary-dropdown">
  <label class="boundary-label" for="boundarySelect">Boundaries</label>
  <div class="boundary-controls">
    <select id="boundarySelect">
      <option value="">-- Select Boundary --</option>
    </select>
    <button id="zoomBoundaryBtn" class="zoom-btn" title="Zoom to Boundary">👁️</button>
  </div>
</div>

    `;
  layersList.appendChild(boundaryDropdownWrapper);

  const boundarySelect =
    boundaryDropdownWrapper.querySelector("#boundarySelect");
  const zoomBoundaryBtn =
    boundaryDropdownWrapper.querySelector("#zoomBoundaryBtn");

  // --- Loop through layers ---
  sortedLayers.forEach(([layerType, spec]) => {
    const isBoundary = layerType.startsWith("boundary_");

    if (isBoundary) {
      // Add boundary as dropdown option
    ...
    <button class="layer-btn" data-layer="${layerType}">
        ...
        <div style="display: flex; flex-direction: column; align-items: flex-start;">
            <span class="label" style="margin-top: 1px; margin-bottom: 1px;">${spec.label}</span>
            <span class="measurement" id="measurement-${layerType}" style="margin-top: 1px; margin-bottom: 1px;"></span>
        </div>
    </button>
    <button class="edit-btn" data-layer="${layerType}" title="Edit Features">✏️</button>
    <button class="delete-btn" data-layer="${layerType}" title="Delete Features">🗑️</button>
    <button class="color-btn" data-layer="${layerType}" title="Change Color">🎨</button>
</div>
            `;
      ...
      colorButton.addEventListener("click", () =>
        openChangeColorPopup(layerType)
      );
  ...
  // --- Handle zoom for selected boundary ---
  zoomBoundaryBtn.addEventListener("click", () => {
    const selectedLayer = boundarySelect.value;
    if (selectedLayer) {
      zoomToBoundary(selectedLayer);
    } else {
      alert("Please select a boundary to zoom.");
    }
  });
}

function updateMeasurementDisplay(layerType) {
  console.log(`Updating measurement display for layer: ${layerType}`);
  const spec = LAYERS_SPECS[layerType];
  ...
  if (measurementEl) {
    ...
    setTimeout(() => {
      measurementEl.textContent = layerType.includes("boundary")
        ? ""
        : measurement > 0
        ? `${measurement.toFixed(2)} ${
            spec.geometryType === "Polygon"
              ? "sq ft"
              : spec.geometryType === "LineString"
              ? "ft"
              : "N"
          }`
        : `0 ${
      ...
      console.log(
        `Updated measurement for ${layerType}: ${measurement.toFixed(2)} ${
          spec.geometryType === "Polygon"
            ? "sq ft"
            : spec.geometryType === "LineString"
            ? "ft"
...
function initControls() {
  ...
  document
    ...
    .addEventListener("click", toggleBasemapLabels);
}
...
function cancelDrawing() {
  ...
  if (selectedLayer) {
    document
      .querySelector(`button.layer-btn[data-layer="${selectedLayer}"]`)
...
function cancelDeleting() {
  ...
  if (deleteModeLayer) {
    document
      .querySelector(`button.delete-btn[data-layer="${deleteModeLayer}"]`)
      .classList.remove("active");
    deleteModeLayer = null;
  }
  document.getElementById("drawingNotification").classList.remove("visible");
...
function cancelEditing() {
  ...
  if (editModeLayer) {
    document
      .querySelector(`button.edit-btn[data-layer="${editModeLayer}"]`)
      .classList.remove("active");
    editModeLayer = null;
  }
  document.getElementById("drawingNotification").classList.remove("visible");
...
function startDrawing(layerType) {
  ...
  document
    .querySelector(`button.layer-btn[data-layer="${layerType}"]`)
  ...
  draw.on("drawend", (event) => {
    ...
    if (spec.geometryType === "Polygon") {
      ...
      measurement =
        turf.length(turf.lineString(coords), { units: "meters" }) * 3.28084; // ft
    } else if (spec.geometryType === "Point") {
      measurement = 1;
    }
    console.log(
      `Drawing ${layerType}: Adding measurement ${measurement.toFixed(2)}`
    );
...
function startDeleting(layerType) {
  ...
  const deleteButton = document.querySelector(
    `button.delete-btn[data-layer="${layerType}"]`
  );
  ...
  select.on("select", (event) => {
    ...
    if (selectedFeatures.length > 0) {
      ...
      console.log(
        `Deleting ${layerType}: Subtracting measurement ${measurement.toFixed(
          2
        )}`
      );
      source.removeFeature(feature);
...
function startEditing(layerType) {
  ...
  const editButton = document.querySelector(
    `button.edit-btn[data-layer="${layerType}"]`
  );
  ...
  modify.on("modifystart", (event) => {
    ...
    if (features.length > 0) {
      ...
      console.log(
        `Edit started for ${layerType}: Feature ID ${features[0].getId()}, Old coordinates:`,
  ...
  modify.on("modifyend", (event) => {
    ...
    if (features.length > 0 && currentFeature) {
      ...
      console.log(
        `Edit ended for ${layerType}: Feature ID ${featureId}, New coordinates:`,
        geometry.getCoordinates()
      );
      if (spec.geometryType === "Polygon") {
        const oldCoords = currentFeature
          .getGeometry()
          .getCoordinates()[0]
          .map((c) => ol.proj.toLonLat(c));
        const newCoords = geometry
          .getCoordinates()[0]
          .map((c) => ol.proj.toLonLat(c));
        if (!isValidPolygon([newCoords])) {
          ...
          measurements[layerType] =
            ...
            calculateMeasurement(currentFeature, spec);
        ...
        const newCoords = geometry
          ...
          .map((c) => ol.proj.toLonLat(c));
        oldMeasurement =
          turf.length(turf.lineString(oldCoords), { units: "meters" }) *
          3.28084; // ft
        newMeasurement =
          turf.length(turf.lineString(newCoords), { units: "meters" }) *
          3.28084; // ft
      } else if (spec.geometryType === "Point") {
        oldMeasurement = 1;
        newMeasurement = 1;
      ...
      console.log(
        `Editing ${layerType}: Old measurement ${oldMeasurement.toFixed(
          2
        )}, New measurement ${newMeasurement.toFixed(2)}, Current total ${
          measurements[layerType]?.toFixed(2) || 0
        }`
      );
      measurements[layerType] =
        (measurements[layerType] || 0) - oldMeasurement + newMeasurement;
      if (measurements[layerType] < 0) measurements[layerType] = 0;
      updateMeasurementDisplay(layerType);
      console.log(
        `After edit, total measurement for ${layerType}: ${measurements[
          layerType
        ].toFixed(2)}`
      );
      history.push({
      ...
      console.warn(
        `No features modified or currentFeature not set in ${layerType}`
      );
    }
  });
  ...
  notification.textContent =
    "Click and drag to edit a feature. Press ESC to cancel.";
  notification.classList.add("visible");
}
...
function undo() {
  ...
  const { layerType, featureId, action, feature, oldFeature, newFeature } =
    lastAction;
  const source = vectorLayers[layerType].getSource();
  const spec = LAYERS_SPECS[layerType];
  if (action === "add" && source.getFeatureById(featureId)) {
    ...
    console.log(
      `Undoing add for ${layerType}: Subtracting measurement ${measurement.toFixed(
        2
      )}`
    );
    source.removeFeature(feature);
    ...
    console.log(
      `Undoing delete for ${layerType}: Adding measurement ${measurement.toFixed(
        2
      )}`
    );
    measurements[layerType] = (measurements[layerType] || 0) + measurement;
    ...
    if (feature) {
      ...
      console.log(
        `Undoing edit for ${layerType}: Current measurement ${currentMeasurement.toFixed(
          2
        )}, Restoring ${oldMeasurement.toFixed(2)}`
      );
      measurements[layerType] =
        (measurements[layerType] || 0) - currentMeasurement + oldMeasurement;
      if (measurements[layerType] < 0) measurements[layerType] = 0; // Ensure measurement doesn't go negative
      updateMeasurementDisplay(layerType);
      redoStack.push({
        layerType,
...
function redo() {
  ...
  const { layerType, feature, featureId, action, oldFeature, newFeature } =
    nextAction;
  const source = vectorLayers[layerType].getSource();
  const spec = LAYERS_SPECS[layerType];
  if (action === "add") {
    ...
    console.log(
      `Redoing add for ${layerType}: Adding measurement ${measurement.toFixed(
        2
      )}`
    );
    measurements[layerType] = (measurements[layerType] || 0) + measurement;
    ...
    if (featureToDelete) {
      ...
      console.log(
        `Redoing delete for ${layerType}: Subtracting measurement ${measurement.toFixed(
          2
        )}`
      );
    ...
    if (feature) {
      ...
      console.log(
        `Redoing edit for ${layerType}: Current measurement ${currentMeasurement.toFixed(
          2
        )}, Applying ${newMeasurement.toFixed(2)}`
      );
      measurements[layerType] =
        (measurements[layerType] || 0) - currentMeasurement + newMeasurement;
      if (measurements[layerType] < 0) measurements[layerType] = 0; // Ensure measurement doesn't go negative
      updateMeasurementDisplay(layerType);
      history.push({
        layerType,
  ...
  Object.entries(vectorLayers).forEach(([layerType, layer]) => {
    ...
    source.getFeatures().forEach((feature) => {
      ...
      clone.setProperties({
        measurement:
          measurement.toFixed(2) +
          (LAYERS_SPECS[layerType].geometryType === "Polygon"
            ? " sqft"
            : LAYERS_SPECS[layerType].geometryType === "LineString"
            ? " ft"
            : " N"),
      });
      features.push(clone);
  ...
  if (confirm("Are you sure you have completed the data correctly?")) {
    dataref
      .update({
        ...
        layers: layerIds,
        totalAreaBoundaryMeasurement: {
          acres: Object.entries(measurements)
            .reduce(
              (sum, [layerType, m]) =>
                sum +
                (LAYERS_SPECS[layerType]?.geometryType === "Polygon"
...
function addCustomLayer() {
  ...
  const icon =
    layerType === "Point" ? "○" : layerType === "LineString" ? "╱" : "■";
  ...
  const activeLayers = Object.fromEntries(
    Object.entries(LAYERS_SPECS).filter(
      ([layerType]) => vectorLayers[layerType]
    )
  );
