...
function initMap() {
    ...
    dataref.once("value").then(async function (snapshot) {
        ...
        boundaryLayers.forEach((boundary, index) => {
            try {
                if (boundary.geometry.type === 'Polygon' && !isValidPolygon(boundary.geometry.coordinates)) {
                    console.warn(`Invalid boundary polygon at index ${index + 1}, skipping:`, boundary);
                    return;
                }
                ...
                const vectorLayer = new ol.layer.Vector({
                    ...
                    style: new ol.style.Style({
                        ...
                        fill: new ol.style.Fill({
                            color: boundary.properties?.fill + (boundary.properties?.['fill-opacity'] ? Math.round(boundary.properties['fill-opacity'] * 255).toString(16).padStart(2, '0') : '40')
                        })
                    })
                });
                vectorLayers[layerType] = vectorLayer;
                ...
                extent = extent ? ol.extent.extend(extent, featureExtent) : featureExtent;
            } catch (error) {
                console.error(`Error processing boundary at index ${index + 1}:`, error, boundary);
            }
        });
        ...
        const boundaryLayersMap = Object.fromEntries(Object.entries(LAYERS_SPECS).filter(([key]) => key.startsWith('boundary_')));
...
function changeLayerColor() {
    ...
    const allLayers = {
        ...boundaryLayersMap,
        ...Object.fromEntries(Object.entries(LAYERS_SPECS).filter(([layerType]) => vectorLayers[layerType] && !layerType.startsWith('boundary_')))
    };
    initLayersList(allLayers);
...
function initLayersList(activeLayers) {
  ...
  <div class="boundary-controls">
    ...
    const boundarySelect = boundaryDropdownWrapper.querySelector('#boundarySelect');
    const zoomBoundaryBtn = boundaryDropdownWrapper.querySelector('#zoomBoundaryBtn');
...
function updateMeasurementDisplay(layerType) {
    ...
    if (measurementEl) {
        ...
        setTimeout(() => {
            measurementEl.textContent = layerType.includes("boundary") ? "" : measurement > 0
