      ...
      if (boundary) {
        try {
          if (
            ...
            !isValidPolygon(boundary.coordinates)
          ) {
      ...
      if (data.status === "Pending" && boundary) {
        try {
          ...
          Object.entries(geojsonData.features).forEach(([index, feature]) => {
            ...
            if (!vectorLayers[layerType]) {
              console.warn(
                `Layer ${layerType} not found for Overpass feature, skipping.`
              );
              return;
            }
            if (
              feature.geometry.type === "Polygon" &&
              !isValidPolygon(feature.geometry.coordinates)
            ) {
              console.warn(
                `Invalid polygon in Overpass data, skipping:`,
                feature
              );
              return;
            }
          ...
          console.error(
            "Error fetching or processing Overpass API data:",
            error
          );
        }
      }
...
function updateMeasurementDisplay(layerType) {
  ...
  if (measurementEl && measurements[layerType]) {
    measurementEl.textContent = `${measurements[layerType].toFixed(2)} ${
